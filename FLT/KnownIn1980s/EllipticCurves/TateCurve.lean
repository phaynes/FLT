...
/-- The Tate parameter is fixed by every `k`-algebra automorphism of the separable closure.
This is the first algebraic input for the Galois equivariance of Tate uniformisation. -/
theorem WeierstrassCurve.qUnitSepClosure_galois (σ : Ω ≃ₐ[k] Ω) :
    Units.map σ.toAlgHom.toRingHom.toMonoidHom (E.qUnitSepClosure Ω) =
      E.qUnitSepClosure Ω := by
  apply Units.ext
  simp [qUnitSepClosure, qUnit]

omit [E.IsMinimal 𝒪[k]] [IsSepClosed Ω] [Algebra.IsSeparable k Ω] in
/-- Consequently a `k`-algebra automorphism preserves the cyclic subgroup generated by
the Tate parameter. -/
theorem WeierstrassCurve.zpowers_qUnitSepClosure_galois (σ : Ω ≃ₐ[k] Ω) :
    (Subgroup.zpowers (E.qUnitSepClosure Ω)).map
        (Units.mapEquiv σ.toRingEquiv.toMulEquiv) =
      Subgroup.zpowers (E.qUnitSepClosure Ω) := by
  rw [MonoidHom.map_zpowers]
  congr 1
  exact E.qUnitSepClosure_galois Ω σ

omit [E.IsMinimal 𝒪[k]] [IsSepClosed Ω] [Algebra.IsSeparable k Ω] in
/-- The induced Galois action on the multiplicative Tate quotient. -/
noncomputable def WeierstrassCurve.tateQuotientGalois (σ : Ω ≃ₐ[k] Ω) :
    Additive (Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)) ≃+
      Additive (Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)) :=
  MulEquiv.toAdditive <| QuotientGroup.congr
    (Subgroup.zpowers (E.qUnitSepClosure Ω))
    (Subgroup.zpowers (E.qUnitSepClosure Ω))
    (Units.mapEquiv σ.toRingEquiv.toMulEquiv)
    (E.zpowers_qUnitSepClosure_galois Ω σ)

omit [E.IsMinimal 𝒪[k]] [IsSepClosed Ω] [Algebra.IsSeparable k Ω] in
@[simp]
theorem WeierstrassCurve.tateQuotientGalois_apply (σ : Ω ≃ₐ[k] Ω) (u : Ωˣ) :
    E.tateQuotientGalois Ω σ (Additive.ofMul
      (u : Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω))) =
      Additive.ofMul
        ((Units.map σ.toAlgHom.toRingHom.toMonoidHom u) :
          Ωˣ ⧸ Subgroup.zpowers (E.qUnitSepClosure Ω)) := by
  rfl
