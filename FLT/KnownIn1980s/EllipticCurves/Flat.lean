...
`F * Φ n + G * ΨSq n = 1`. This is the form the applications consume. The proof does
not require the stronger universal resultant formula: if the generated ideal were proper,
put it inside a maximal ideal of `R₀[X]`. In the quotient field, the image of `X` would be
a common root of the two division polynomials of the base-changed nonsingular curve,
contradicting `isCoprime_Φ_ΨSq_field`. -/
