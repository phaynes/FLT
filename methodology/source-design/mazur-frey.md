...
| full rational 2-torsion | the three roots `0`, `A`, `-B` are rational and distinct | construction API open |
...
- an explicit four-element rational 2-torsion embedding for the Frey model;
...
1. an explicit `FreyCurveSemistableContract` with a concrete local-reduction vocabulary;
2. an `EllipticCurveQuotientIsogenyContract` for a Galois-stable subgroup of prime order;
3. a `SemistableReducibleCharacterDichotomyContract` matching SRC-021; and
4. a finite rational-torsion cardinality contract that cannot be satisfied vacuously by infinite
   torsion.

Only after those four signatures elaborate and are source-reviewed should `FreyPackage.mazur` move
