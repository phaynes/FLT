...
| `FLT-TATE-UNRAMIFIED` | ACTIVE | Four Galois-action helpers, Tate-curve ellipticity, and the concrete/formal reciprocal-j bridge are kernel-clean. Methodology probes now close the formal-product/q-expansion identity, substitution evaluation, both concrete inverse laws, and split-multiplicative reduction of the explicit Tate curve with standard axioms. The local-form constructor is signature-green and reduces exactly to equal-j quadratic-form classification plus exclusion of a second split quadratic twist; those two source theorems and provider migration remain, with nine provider admissions still live |
