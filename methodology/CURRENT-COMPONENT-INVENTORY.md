...
| `FLT-TATE-UNRAMIFIED` | ACTIVE | Four Galois-action helpers, Tate-curve ellipticity, and the concrete/formal reciprocal-j bridge are kernel-clean. Methodology probes now close the formal-product/q-expansion identity, substitution evaluation, both concrete inverse laws, and split-multiplicative reduction of the explicit Tate curve with standard axioms; nine provider admissions remain, with migration and local-field same-j form classification next |
