...
| `FLT-TATE-UNRAMIFIED` | ACTIVE | Four Galois-action helpers, Tate-curve ellipticity, and the concrete/formal reciprocal-j bridge are kernel-clean. Methodology probes now close the formal-product/q-expansion identity, substitution evaluation, both concrete inverse laws, and split-multiplicative reduction of the explicit Tate curve with standard axioms. The local-form constructor reduces exactly to equal-j quadratic-form classification plus exclusion of a second split quadratic twist, and the general `tateEquiv` integration is proved mechanical once the explicit Tate-curve equivalence and variable change exist. Those two local source theorems, the explicit `tateCurveEquiv` construction, and provider migration remain, with nine provider admissions still live |
