...
| `FLT-TATE-UNRAMIFIED` | ACTIVE | Four Galois-action helpers, Tate-curve ellipticity, and the reciprocal-j bridge are kernel-clean. Methodology probes close the formal-product/q-expansion identity, substitution evaluation, inverse laws, and split reduction. The frozen repository already defines formal Tate `X/Y` coordinates and proves their Weierstrass equation. New exact contracts show that concrete local-field coordinates plus the addition law and surjectivity mechanically yield the point homomorphism, exact kernel `q^Z`, quotient equivalence, and general-curve assembly. Silverman V.5.3 is the direct local-form source route, with the two-twist decomposition as fallback. Concrete X/Y evaluation, addition, surjectivity, the exact V.5.3 scope audit, functoriality, and provider migration remain; nine provider admissions are still live |
