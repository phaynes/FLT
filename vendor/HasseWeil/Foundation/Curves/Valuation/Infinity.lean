...
theorem ofFractionRing_sq (r : FractionRing (Polynomial F)) :
    RatFunc.ofFractionRing (r ^ 2) = (RatFunc.ofFractionRing r : RatFunc F) ^ 2 := by
  rw [sq, RatFunc.ofFractionRing_mul, sq]

theorem ofFractionRing_ne_zero {r : FractionRing (Polynomial F)} (hr : r ≠ 0) :
...
theorem intDegree_ofFractionRing_eq_of_surj
...
theorem ordAtInfty_basis_fracPolyX_lift
...
theorem ordAtInfty_basis_fracPolyX_summand_ne_zero
    {r₁ r₂ : FractionRing (Polynomial F)} {d p₁' p₂' : Polynomial F}
...
theorem ordAtInfty_basis_fracPolyX_natDegree_arith
    {r₁ r₂ : FractionRing (Polynomial F)} {d p₁' p₂' : Polynomial F}
...
theorem min_add_add_right_withTop (a b c : WithTop ℤ) :
...
theorem ordAtInfty_smul_coordY_add_ge_min (q₁ q₂ : FractionRing (Polynomial F)) :
...
theorem intDegree_algebraMap_div_algebraMap
...
theorem algebraMap_div_sub_C_eq {n d : Polynomial F} (hd : d ≠ 0) (lam : F) :
...
theorem degree_sub_C_leadingCoeff_div_mul_lt {n d : Polynomial F}
...
theorem ratFunc_exists_C_sub_intDegree_neg {r : RatFunc F}
    (hr : r.intDegree ≤ 0) :
...
theorem natDegree_zero_of_ordAtInfty_nonneg {u : C.CoordinateRing}
    (hu : (0 : WithTop ℤ) ≤
      C.ordAtInfty (algebraMap C.CoordinateRing C.FunctionField u))
...
theorem q_eq_zero_of_norm_natDeg_zero {p q : Polynomial F}
...
noncomputable def fiberQuadratic (a : F) : Polynomial F :=
  Polynomial.X ^ 2 +
    Polynomial.C (C.toAffine.a₁ * a + C.toAffine.a₃) * Polynomial.X -
    Polynomial.C (a ^ 3 + C.toAffine.a₂ * a ^ 2 + C.toAffine.a₄ * a + C.toAffine.a₆)

theorem fiberQuadratic_natDegree (a : F) : (C.fiberQuadratic a).natDegree = 2 := by
  simp only [fiberQuadratic]
  compute_degree!

theorem fiberQuadratic_ne_zero (a : F) : C.fiberQuadratic a ≠ 0 := by
  intro h
  have := C.fiberQuadratic_natDegree a
  rw [h, Polynomial.natDegree_zero] at this
  exact absurd this (by decide)

theorem fiberQuadratic_isRoot_of_smoothPoint {a : F} {P : C.SmoothPoint}
...
noncomputable def coordEval (P : C.SmoothPoint) :
    C.CoordinateRing →+* F :=
  AdjoinRoot.lift (Polynomial.evalRingHom P.x) P.y <| by
    rw [Polynomial.eval₂_evalRingHom]
    exact P.nonsingular.1

theorem coordEval_mk (P : C.SmoothPoint) (g : Polynomial (Polynomial F)) :
    C.coordEval P
...
theorem coordEval_smul_basis (P : C.SmoothPoint) (p q : Polynomial F) :
...
theorem XClass_mem_ker_coordEval (P : C.SmoothPoint) :
...
theorem YClass_mem_ker_coordEval (P : C.SmoothPoint) :
...
theorem maximalIdealAt_le_ker_coordEval (P : C.SmoothPoint) :
...
theorem ker_coordEval_ne_top (P : C.SmoothPoint) :
    RingHom.ker (C.coordEval P) ≠ ⊤ := by
  rw [Ne, Ideal.eq_top_iff_one, RingHom.mem_ker, map_one]
  exact one_ne_zero

theorem ker_coordEval_eq (P : C.SmoothPoint) :
