...
private lemma monomial_square_mul_mul (A B C t : k) (a b c : ℕ) :
    (A * t ^ a) ^ 2 * (B * t ^ b) * (C * t ^ c) =
...
private lemma monomial_mul_cube (A B t : k) (a b : ℕ) :
...
private lemma monomial_mul_mul_square (A B C t : k) (a b c : ℕ) :
    (A * t ^ a) * (B * t ^ b) * (C * t ^ c) ^ 2 =
